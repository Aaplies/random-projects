...
std::string newPrintTime(float input) // this func written in 4th hour 18 Oct. 2023
{
    bool ago = input < 0;
    if (ago)
        input *= -1; // don't make input const or a reference!
    const int32_t seconds = int(round(input * 3600));
    std::string S_return = std::to_string(int(floor(seconds / 86400)) % 365) + "d ";
    S_return += std::to_string(int(floor(seconds / 3600)) % 24) + "h ";
    S_return += std::to_string(int(floor(seconds / 60)) % 60) + "m ";
    S_return += std::to_string(seconds % 60) + "s";
    if (ago)
        S_return += " ago";
    return S_return;
}
...
int main()
{
    ...
    std::cout << "\nAbsolute next predicted event:  " << newPrintTime(averageDifferenceHours - hoursList[0]) << '\n';
    float absNextTime = abs(averageDifferenceHours - hoursList[0]);

    tempAvgDiff = averageDifferenceHours - hoursList[0]; // averageDifferenceHours gets chhanged in the coming lines so this saves it
    ...
    if (averageDifferenceHours == 0)
        std::cout << "\nRealistic next predicted event: undefined";
    else
        std::cout << "\nRealistic next predicted event: " << newPrintTime(tempAvgDiff) << '\n';

    std::cout << "\n\nAD = " << newPrintTime(averageMean) << ".\n";
    std::cout << "SD = " << newPrintTime(standardDeviation) << ".\n";
    std::cout << "Mean = " << newPrintTime(averageDifferenceHours) << ".\n";
    ...
    std::cout << "STC: " << newPrintTime(averageDifferenceHours - prevAvgDiff) << " (" << newPrintTime(averageDifferenceHours) << ")\n";
    std::cout << "Irregular: ";
